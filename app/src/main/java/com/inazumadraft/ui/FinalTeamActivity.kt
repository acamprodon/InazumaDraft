...
class FinalTeamActivity : AppCompatActivity() {
    ...
    private fun bindSlotView(view: View, i: Int) {
        ...
        if (p != null) {
            img.loadPlayerImage(p.image); name.text = p.nickname; elem.loadPlayerImage(p.element)
            ...
            name.text = codeToNice(toCode(formation.positions[i])); img.loadPlayerImage(null); elem.loadPlayerImage(null); img.background = null
            view.setOnClickListener {
